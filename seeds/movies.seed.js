//WRITE THE CODE NEC. TO SEED THE DB

const mongoose = require("mongoose");
//first, require from models
const Movie = require("../models/Movie.model");

//second, connect to db
require("../db");

//third (what needs to be written on the db)
const movies = [
  {
...
//4th, create script that will be run to actually seed the database
//creating the movies in our db
Movie.create(movies)
  .then((createdMovies) => {
    console.log(`Created ${createdMovies.length} movies`);

    //once they are created, close the connection (7:42)
    mongoose.connection.close();
  })
  .catch((error) =>
    console.log("An error occurred on the movie creation", error)
  );
